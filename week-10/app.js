...
const formatItem = (item) => {
  return `${item.id.toString().padStart(3)} | ${item.nombre.padEnd(20)} | ${item.tipo.padEnd(10)} | $${item.precio.toString().padStart(5)} | ${item.disponible ? 'Disponible' : 'No disponible'} | ${item.descripcion ?? 'N/A'}`;
};
...
console.log("Actualizado precio de Guitarra Acústica a $550");
console.log("");

// Paso 5: Estadísticas
const stats = calculateStats('precio');
console.log("Estadísticas de precios:");
console.log(`Mín: $${stats.min}, Máx: $${stats.max}, Prom: $${stats.avg.toFixed(2)}, Total: $${stats.total}`);
console.log("");
